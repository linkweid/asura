...
local AsuraLogHandler = BasePlugin:extend()

AsuraLogHandler.PRIORITY = 3111
AsuraLogHandler.VERSION = "0.1.0"
...
local function generate_post_payload(method, content_type, parsed_url, body)
  ...
  ngx.log(ngx.ERR, "req_body lcs111: ", "lcs")
  local ctx = ngx.ctx
  if ctx.asura then
    req_body = ctx.asura.req_body
    res_body = ctx.asura.res_body
    ngx.log(ngx.ERR, "req_body failed to create timer: ", req_body)
    ngx.log(ngx.ERR, "res_body failed to create timer: ", res_body)
    body = body .. req_body
    body = body .. res_body
  end
...
function AsuraLogHandler:new(name)
    AsuraLogHandler.super.new(self, name or "http-log")
end
...
function AsuraLogHandler:serialize(ngx, conf)
  return cjson_encode(basic_serializer.serialize(ngx))
end

function AsuraLogHandler:access()
    AsuraLogHandler.super.access(self)
  ngx.ctx.asura = {req_body = req_get_body_data()}
end
function AsuraLogHandler:body_filter(conf)
    AsuraLogHandler.super.body_filter(self)
    local chunk = ngx.arg[1]
    local ctx = ngx.ctx
    local res_body = ctx.asura and ctx.asura.res_body or ""
    res_body = res_body .. (chunk or "")
    ctx.asura.res_body = res_body
end

function AsuraLogHandler:log(conf)
    AsuraLogHandler.super.log(self)
